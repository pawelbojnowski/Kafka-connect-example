...
import static pl.pb.kafkaconnectexample.cassandra.proto.KafkaConfig.runStreams;
...
public class KafkaStreamExample {
	...
	private static void createStream(final StreamsBuilder builder) {
		...
		final KStream<String, User> source = builder.stream(CASSANDRA_SOURCED_USER, consumed);
		source.peek((key, value) -> println(key + " = " + value))
				.mapValues(value -> mapUserToClient(value))
				.to(KafkaConfig.CASSANDRA_SINK_CLIENT, produced);
	}
